...
female_keywords = ["she/her", "girl", "female", "woman", "lady"]
if any(word in bio for word in female_keywords):
await channel.send(f"<@{user_id}> good girl")
else:
...
embed = discord.Embed(title="HWID Submitted", description="Your HWID has been sent to the owner for authentication.", color=discord.Color.green())
...
embed = discord.Embed(title="HWID Submitted (Tester)", description=f"Your HWID has been sent to the Owner (<@{CO_OWNER_ID}>) for tester authentication.", color=discord.Color.green())
await interaction.response.send_message(embed=embed, ephemeral=True)
...
try:
await interaction.user.send("loadstring(game:HttpGet('[https://raw.githubusercontent.com/vqmpjayZ/utils/refs/heads/main/CopyHWID.lua'))(](https://raw.githubusercontent.com/vqmpjayZ/utils/refs/heads/main/CopyHWID.lua'%29%29%28))")
